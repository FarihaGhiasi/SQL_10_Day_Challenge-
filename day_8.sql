...
--Questions
-- 8.1 Obtain the names of all physicians that have performed a medical procedure they have never been certified to 
--perform.
select *
from physician 
...
SELECT ph.name AS physician_name, pr.name AS procedure_name, u.date, p.name AS patient_name
FROM physician ph
JOIN patient p ON ph.employeeid = p.pcp   
...
select name
from physician 
where employeeid in (select t.physician from trained_in t 
                                                join undergoes u
                                                 on t.treatment = u.procedure 
...
select ph.name as physician_name, pr.name as procedure_name, u.date as date_of_procedure,  p.name as patient_name, t.CertificationExpires as certificateion_expiry_date 
from physician ph
join trained_in t on ph.employeeid = t.physician 
join undergoes u on t.treatment = u.procedure 
...
--physician. Show the following information: Patient name, physician name, nurse name (if any), start and end time 
--of appointment, examination room, and the name of the patient's primary care physician.
select p.name as patient, ph.name as physician, n.name as nurse, a.start, a.end, a.examinationroom, php.name AS primary_physician
from appointment a
join patient p on a.patient = p.ssn 
join physician ph on a.physician = ph.employeeid
join physician php on p.pcp = php.employeeid
left join nurse n on a.prepnurse = n.employeeid
where a.physician <> p.pcp;

-- 8.6 The Patient field in Undergoes is redundant, since we can obtain it from the Stay table. There are no 
--constraints in force to prevent inconsistencies between these two tables. More specifically, the Undergoes 
--table may include a row where the patient ID does not match the one we would obtain from the Stay table through 
...
join room r on o.blockcode = r.blockcode
where r.number = 123;     
...
select examinationroom, count(examinationroom) as number_of_appointments 
from appointment 
...
-- 8.9 Obtain the names of all patients and their primary care physician, such that the following are true:
    ...
    -- The patient has had at least two appointments where the nurse who prepared the appointment was a registered 
    --nurse.
...
AND EXISTS (
  SELECT * 
  FROM Undergoes U, Procedures Pr
